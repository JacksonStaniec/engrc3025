...
export const Cogito = () => (
  <>
    ...
    <Text>
      ...
      emotion, speaking rate, periods of uninterrupted speaking, and other metrics. I worked on the
      ...
      The company regularly runs a load testing tool, since their products are used in call centers
      with very large volumes of calls. Their current software could only load test at a constant
      rate, which does not follow actual patterns -- which usually follows a large acceleration of
      calls being placed at the beginning of the day, peaking around midday working hours, and
      declining until evening.
    </Text>
    <Title mt="xs" order={4}>
      Architecture
    </Title>
    <Text>
      ...
      <Center>
        <Image
          component={NextImage}
          src={Sip}
          alt="ESIP internet telephony protocol"
          h="auto"
          maw="50%"
        />
      </Center>
      Previously, these XML documents were manually created, and thus only had one set rate to place
      calls. So, in order to faciliate dynamic call rates, I needed to implement:
    </Text>
    <ol>
    ...
    <Text>
      ...
      minute to follow this rate, given the time constraint and the total number of calls to place.
    </Text>
    <Title mt="xs" order={4}>
      Results
    </Title>
    <Text>
      ...
      to ascertain the correct autoscaling of containers resulting from increasing loads. This new
      capaniltiers preceded a major product release, and the company&apos;s full transition from
      on-prem infrastructure to cloud-based software service.
    </Text>
  </>
);
...
export const Research = () => (
  <>
    ...
    <Text>
      Ease of code modifiability is a prominent concern in good software engineering; software that
      is hard to modify from its original construction will be difficult to maintain and unable to
      effectively adapt to changing business requirements or new desired functionality. One measure
      of modifiability is coupling: the degree of interdependence between modules. Low coupling
      means that modules are fairly independent, while high degrees of coupling means that changing
      one module likely requires changing another module that uses it -- meaning that small changes
      in one module may require multiple changes in several other files. So, the goal of our project
      was to create a tool that allows an arbitrary Python repository to be analyzed for various
      dependency relationships, and provide an interface for users to explore these relationships
      visually.
    </Text>
    <Title mt="xs" order={4}>
      Architecture
...
export const TeachingAssistant = () => (
  <>
    ...
    <Text>
      ...
      necessary for the course to function. However, while the vast majority of student assignments
      are able to be graded, a small number of submissions per project are malformed in such a way
      that the autograder cannot run. Since this results in an automatic zero score for the student,
      we want to avoid these stressful situations whenever possible.
    </Text>
    <Title mt="xs" order={4}>
      Results
    </Title>
    <Text>
      ...
      and testing suite for two new assignments that were created that semester. in Fall 2023.
    </Text>
  </>
);
